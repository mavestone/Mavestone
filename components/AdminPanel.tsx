            ...
            <aside className="w-64 flex-shrink-0 border-r border-white/10 bg-[#0A0A0A] flex flex-col justify-between p-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-white/5 flex items-center justify-between px-2">
                        ...
                        <button onClick={() => { logout(); handleClose(); }} className="text-gray-500 hover:text-red-400"><LogOut size={16} /></button>
                    </div>
                </div>
            </aside>
            ...
            <main className="flex-1 overflow-y-auto bg-black relative">
                <div className="sticky top-0 z-20 bg-black/80 backdrop-blur-md border-b border-white/5 px-8 py-4 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4">
                        {processingImage && <span className="flex items-center gap-2 text-blue-400 text-xs font-bold animate-pulse"><Loader2 size={12} className="animate-spin" /> Processing Media...</span>}
                        ...
                        <div className="space-y-12">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                <div className="p-6 rounded-2xl bg-[#0F0F11] border border-white/5 space-y-4">
                                    ...
                                    <div className="space-y-4">
                                        <label className="text-[10px] text-gray-500 uppercase tracking-widest">Featured Film</label>
                                        ...
                                        <input type="text" placeholder="Hero Label" value={projectConfig.label} onChange={(e) => updateProjectConfig({ label: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none" />
                                    </div>
                                </div>
                                <div className="p-6 rounded-2xl bg-[#0F0F11] border border-white/5 space-y-4">
                    ...
                    {activeTab === 'settings' && (
                        <div className="max-w-2xl mx-auto space-y-6">
                            ...
                            <div className="p-8 rounded-3xl bg-red-500/5 border border-red-500/10">
                                <h4 className="text-red-400 text-xs font-bold uppercase tracking-widest mb-4">System Utilities</h4>
                                <button className="px-6 py-2 bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg text-[10px] font-bold hover:bg-red-500/20 transition-all" onClick={() => { if(confirm("Reset all content to factory defaults?")) useContent().seedDatabase() }}>Restore Default Data</button>
                            </div>
                        </div>
                    )}
                </div>
