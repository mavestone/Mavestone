    ...
    <SectionWrapper id="work">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 text-left w-full relative z-10">
        <div className="items-start text-left max-w-xl">
           <h2 className="text-4xl md:text-7xl font-black tracking-tighter text-white mb-2">Latest Film</h2>
           <p className="text-gray-400 font-medium">A glimpse into our newest cinematic story.</p>
        </div>
        <div className="mt-8 md:mt-0 flex-shrink-0">
            <Link 
                to="/projects" 
                className="group flex items-center gap-2 px-8 py-3 rounded-full bg-white/5 border border-white/10 backdrop-blur-md text-[10px] font-black uppercase tracking-widest text-white hover:bg-white/10 hover:border-white/20 transition-all duration-300"
            >
      ...
      <div className="w-full aspect-video rounded-3xl overflow-hidden bg-black shadow-2xl relative isolate border border-white/5">
        {!isPlaying ? (
            <div 
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col items-center justify-center">
                    ...
                    <div className="absolute bottom-6 left-6 right-6 md:bottom-10 md:left-10 md:right-auto md:max-w-xl transform translate-y-10 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-out delay-200">
                         <div className="glass-panel p-6 md:p-8 rounded-2xl border border-white/10 backdrop-blur-xl bg-black/60 text-left shadow-2xl">
                            <h3 className="text-2xl md:text-3xl font-black text-white mb-2 leading-tight tracking-tight">{latestVideo.title}</h3>
                            <p className="text-gray-300 text-sm md:text-base leading-relaxed font-light line-clamp-3">{latestVideo.description}</p>
                         </div>
                    </div>
                </div>
            </div>
